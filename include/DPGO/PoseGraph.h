...
/**
 * @brief A pose graph class representing the local optimization problem in distributed
 * PGO.
 */
class PoseGraph {
 public:
   ...
   * @return
   */
  std::vector<RelativeSEMeasurement> sharedLoopClosuresWithRobot(
      unsigned neighbor_id) const;
  /**
  ...
  /**
   * @brief Return a copy of all LOCAL measurements (i.e., without inter-robot loop
   * closures)
   * @return
   */
  ...
  /**
   * @brief Construct data matrices that are needed for optimization, if they do not yet
   * exist
  ...
  /**
   * @brief Get the set of neighbor robot IDs that share inter-robot loop closures with
   * me
   * @return
   */
  ...
  /**
   * @brief Return true if the input robot is a neighbor (i.e., share inter-robot loop
   * closure)
   * @param robot_id
   * @return
   */
   ...
   * @param srcID
   * @param dstID
   * @return writable pointer to the desired measurement (nullptr if measurement does
   * not exists)
   */
