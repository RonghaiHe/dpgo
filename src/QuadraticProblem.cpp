...
double QuadraticProblem::f(ROPTLIB::Variable *x) const {
  Eigen::Map<const Matrix> X((double *)x->ObtainReadData(),
                             relaxation_rank(),
...
void QuadraticProblem::EucGrad(ROPTLIB::Variable *x, ROPTLIB::Vector *g) const {
  Eigen::Map<const Matrix> X((double *)x->ObtainReadData(),
                             relaxation_rank(),
                             (dimension() + 1) * num_poses());
  Eigen::Map<Matrix> EG((double *)g->ObtainWriteEntireData(),
                        relaxation_rank(),
                        (dimension() + 1) * num_poses());
  EG = X * pose_graph_->quadraticMatrix() + pose_graph_->linearMatrix();
}

void QuadraticProblem::EucHessianEta(ROPTLIB::Variable *x,
                                     ROPTLIB::Vector *v,
                                     ROPTLIB::Vector *Hv) const {
  Eigen::Map<const Matrix> V((double *)v->ObtainReadData(),
                             relaxation_rank(),
                             (dimension() + 1) * num_poses());
